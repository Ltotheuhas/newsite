    ...
    <v-container>
      ...
      <v-row>
        <v-col cols="12" md="4" v-for="product in products" :key="product._id">
          <v-card class="product-card" @click="openProductModal(product)" @mouseenter="hoveredProduct = product._id" @mouseleave="hoveredProduct = null">
            <v-img :src="getImageUrl(hoveredProduct === product._id && product.images[1] ? product.images[1].asset._ref : product.images[0].asset._ref)" aspect-ratio="1"></v-img>
        ...
        <v-card v-if="selectedProduct">
          ...
          <v-card-text>
            <v-carousel hide-delimiters>
              <v-carousel-item v-for="(image, index) in selectedProduct.images" :key="index" :src="getImageUrl(image.asset._ref)"></v-carousel-item>
            </v-carousel>
            <p>{{ selectedProduct.description }}</p>
  
            <!-- Size Dropdown if Available -->
            <v-select
              v-if="selectedProduct.sizesWithStock && selectedProduct.sizesWithStock.length > 0"
              v-model="selectedSize"
              :items="selectedProduct.sizesWithStock.map(sizeStock => sizeStock.size)"
              label="Select Size"
            ></v-select>
            ...
            <v-row justify="space-between">
              ...
              <v-text-field v-model="quantity" type="number" min="1" :max="selectedSizeStock" class="quantity-input" label="Quantity" readonly></v-text-field>
            ...
            <v-btn
              ...
              color="primary"
              :data-item-id="selectedProduct._id"
              :data-item-name="selectedProduct.name"
              :data-item-price="selectedProduct.price"
              :data-item-url="`https://luhas.gratis/store/${selectedProduct.slug.current}`"
              :data-item-description="selectedProduct.description"
